...
import {
  ...
  ShieldCheck, Banknote, MessageSquare,
} from "lucide-react";
import { useAdmin } from "./AdminContext.jsx";
import { listingFace } from "./listingUtils.js";
        ...
        <aside className={`ad-sidebar ${sidebarOpen ? "open" : ""}`}>
          <div className="ad-sidebar-header">
            <div className="ad-brand">
              ...
              <div>
                <p className="ad-brand-name">STP Tours</p>
  ...
  return (
    <div className="ad-section">
      <div className="ad-section-header">
        ...
        <p>Aqui está o resumo do estado actual do portal STP Tours.</p>
      </div>
...
function SectionCategories() {
  ...
  function openAdd() { setSaveError(""); setForm({ title: "", tagline: "", description: "", icon: "Compass", heroImage: "" }); setModal("add"); }
  function openEdit(cat) { setSaveError(""); setForm({ title: cat.title, tagline: cat.tagline, description: cat.description || "", icon: cat.icon || "Compass", heroImage: cat.heroImage || "" }); setModal(cat); }

  async function handleSave() {
    if (!form.title.trim()) return;
    setSaving(true); setSaveError("");
    try {
      if (modal === "add") await addCategory(form);
      else await updateCategory(modal.id, form);
      setModal(null);
    } catch (err) {
      setSaveError(err.message || "Não foi possível guardar. Verifica se o servidor está a correr.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete(id) {
    try {
      ...
      alert(err.message || "Não foi possível eliminar a categoria.");
    }
  ...
  async function handleToggle(id) {
    try {
      ...
      alert(err.message || "Não foi possível alterar o estado da categoria.");
    }
  }
          ...
          <div key={cat.id} className={`ad-list-item ${cat.ativo ? "" : "ad-list-item--off"}`}>
            <div className="ad-list-item-main">
              <div className="ad-list-avatar">
                {cat.heroImage ? <img src={cat.heroImage} alt={cat.title} /> : cat.title.charAt(0)}
              </div>
...
function SectionServices() {
  ...
  function openAdd() { setSaveError(""); setForm({ name: "", description: "", label: "", icon: "Compass", image: "" }); setModal("add"); }
  function openEdit(svc) { setSaveError(""); setForm({ name: svc.name, description: svc.description, label: svc.label || "", icon: svc.icon || "Compass", image: svc.image || "" }); setModal(svc); }

  async function handleSave() {
    if (!form.name.trim()) return;
    setSaving(true); setSaveError("");
    try {
      if (modal === "add") await addService(selectedCat, form);
      else await updateService(selectedCat, modal.id, form);
      setModal(null);
    } catch (err) {
      setSaveError(err.message || "Não foi possível guardar o serviço.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete(id) {
    try {
      await deleteService(selectedCat, id);
    } catch (err) {
      alert(err.message || "Não foi possível eliminar o serviço.");
    }
    setConfirmDelete(null);
  }
  ...
  return (
    <div className="ad-section">
        ...
        <div className="ad-list">
          ...
          {cat.services?.map((svc) => (
            <div key={svc.id} className="ad-list-item">
              <div className="ad-list-item-main">
                <div className="ad-list-avatar ad-list-avatar--blue">
                {svc.image ? <img src={svc.image} alt={svc.name} /> : svc.name.charAt(0)}
              </div>
                <div className="ad-list-info">
                  <h4>{svc.name}</h4>
                  <p>{svc.description}</p>
      ...
      {modal !== null && (
        <FormModal title={modal === "add" ? "Novo serviço" : "Editar serviço"} onClose={() => setModal(null)} onSave={handleSave} saving={saving} error={saveError}>
...
function SectionPartners() {
  ...
  const [saving, setSaving] = useState(false);
  const [saveError, setSaveError] = useState("");
  const [galleryFor, setGalleryFor] = useState(null); // listing cuja galeria está aberta

  function changeCat(id) { setSelectedCat(id); }

  function openAdd() { setSaveError(""); setForm(emptyForm); setModal("add"); }
  function openEdit(l) {
    setSaveError("");
    setForm({
  ...
  async function handleSave() {
    ...
    setSaving(true); setSaveError("");
    try {
      if (modal === "add") await addListing(selectedCat, selectedSvc, payload);
      else await updateListing(selectedCat, selectedSvc, modal.id, payload);
      setModal(null);
    } catch (err) {
      setSaveError(err.message || "Não foi possível guardar o parceiro.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete(id) {
    try {
      await deleteListing(selectedCat, selectedSvc, id);
    } catch (err) {
      alert(err.message || "Não foi possível eliminar o parceiro.");
    }
    setConfirmDelete(null);
  }
  ...
  return (
    <div className="ad-section">
        ...
        <div className="ad-list" style={{ marginTop: 20 }}>
          ...
          {svc.listings?.map((l) => (
            <div key={l.id} className="ad-list-item">
              <div className="ad-list-item-main">
                <div className="ad-list-avatar ad-list-avatar--blue">
                  {listingFace(l) ? <img src={listingFace(l)} alt={l.nome} /> : (l.nome || "?").charAt(0)}
                </div>
                <div className="ad-list-info">
              ...
              <div className="ad-list-actions">
                <button className="ad-icon-btn" title="Gerir fotos" onClick={() => setGalleryFor(l)}><ImageIcon size={16} /></button>
      ...
      {modal !== null && (
        <FormModal title={modal === "add" ? "Novo parceiro" : "Editar parceiro"} onClose={() => setModal(null)} onSave={handleSave} saving={saving} error={saveError} wide>
      ...
      {galleryFor && (
        <GalleryModal listing={galleryFor} onClose={() => setGalleryFor(null)} />
      )}
...
function SectionBookings() {
  const { reservas, updateReservaStatus, aprovarReserva, confirmarPagamentoReserva } = useAdmin();
  const [filter, setFilter] = useState("todas");
  const [search, setSearch] = useState("");
  const [approveModal, setApproveModal] = useState(null); // reserva a aprovar
  const [approveForm, setApproveForm] = useState({ mensagem: "", valorSinal: "" });
  const [saving, setSaving] = useState(false);
  const [saveError, setSaveError] = useState("");
  ...
  async function handleStatusChange(id, status) {
    try {
      ...
      alert(err.message || "Não foi possível atualizar a reserva.");
    }
  }

  function openApprove(r) {
    setSaveError("");
    setApproveForm({ mensagem: `Boas notícias! Confirmámos disponibilidade para a tua reserva. Para garantirmos o lugar, pedimos o pagamento de um sinal de 50%.`, valorSinal: "" });
    setApproveModal(r);
  }

  async function handleApprove() {
    if (!approveForm.mensagem.trim()) { setSaveError("Escreve uma mensagem para o cliente."); return; }
    setSaving(true); setSaveError("");
    try {
      await aprovarReserva(approveModal.id, { mensagem: approveForm.mensagem, valorSinal: approveForm.valorSinal, percentualSinal: 50 });
      setApproveModal(null);
    } catch (err) {
      setSaveError(err.message || "Não foi possível aprovar a reserva.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleConfirmPayment(id) {
    if (!confirm("Confirmas que recebeste o sinal deste cliente? A reserva fica 100% confirmada.")) return;
    try {
      await confirmarPagamentoReserva(id);
    } catch (err) {
  ...
  return (
    <div className="ad-section">
      <div className="ad-section-header">
        <div>
          <h2>Reservas</h2>
          <p>Fluxo: pedido → verificas condições e aprovas (envia mensagem) → cliente paga o sinal → confirmas o pagamento e a reserva fica marcada.</p>
        </div>
      </div>

      {/* Filtros */}
      <div className="ad-filter-tabs">
        {[["todas", "Todas"], ["pendente", `Pendentes (${counts.pendente})`], ["aprovada", `Aguardam sinal (${counts.aprovada})`], ["confirmada", `Confirmadas (${counts.confirmada})`], ["cancelada", `Canceladas (${counts.cancelada})`]].map(([key, label]) => (
            ...
            <div key={r.id} className="ad-list-item ad-booking-item">
              <div className="ad-list-item-main">
                ...
                <div className="ad-list-info">
                  ...
                  {r.status === "aprovada" && (
                    <div className="ad-deposit-box">
                      <p><ShieldCheck size={13} /> Mensagem enviada: <em>"{r.adminMessage}"</em></p>
                      {r.depositValue && <p><Banknote size={13} /> Sinal pedido: <strong>{r.depositValue}</strong> ({r.depositPercent}%)</p>}
                      <p>
                        {r.clientReportedPayment
                          ? "✅ O cliente assinalou que já pagou — confirma para fechar a reserva."
                          : "⏳ A aguardar que o cliente pague o sinal."}
                      </p>
                    </div>
                  )}
                  {r.status === "confirmada" && r.depositPaidAt && (
                    <p className="ad-list-meta">Sinal confirmado em {new Date(r.depositPaidAt).toLocaleString("pt-PT")}</p>
                  )}
                </div>
              </div>
              <div className="ad-list-actions ad-list-actions--col">
                {r.status === "pendente" && (
                  <>
                    <button className="ad-action-btn confirm" onClick={() => openApprove(r)}>
                      <ShieldCheck size={14} /> Aprovar (há condições)
                    </button>
                    <button className="ad-action-btn cancel" onClick={() => handleStatusChange(r.id, "cancelada")}>
                ...
                {r.status === "aprovada" && (
                  <>
                    <button className="ad-action-btn confirm" onClick={() => handleConfirmPayment(r.id)}>
                ...
                {r.status === "cancelada" && (
                  <button className="ad-action-btn confirm" onClick={() => handleStatusChange(r.id, "pendente")}>
                    <Clock size={14} /> Repor
                  </button>
                )}
              </div>
      ...
      {approveModal && (
        <FormModal
          title={`Aprovar reserva de ${approveModal.name}`}
          onClose={() => setApproveModal(null)}
          onSave={handleApprove}
          saving={saving}
          error={saveError}
        >
          <p style={{ fontSize: "0.85rem", color: "var(--ad-muted)", margin: "0 0 4px" }}>
            Confirma que há condições para avançar (disponibilidade, datas, etc.) e escreve a mensagem que o cliente vai ver, com os próximos passos para pagar o sinal.
          </p>
          <Field
            label="Mensagem para o cliente *"
            value={approveForm.mensagem}
            onChange={(v) => setApproveForm((p) => ({ ...p, mensagem: v }))}
            placeholder="Ex: Confirmámos disponibilidade! Para garantir a reserva, paga um sinal de 50%..."
            textarea
          />
          <Field
            label="Valor do sinal (50%)"
            value={approveForm.valorSinal}
            onChange={(v) => setApproveForm((p) => ({ ...p, valorSinal: v }))}
            placeholder="Ex: 90€"
          />
        </FormModal>
      )}
...
function StatusBadge({ status }) {
  const map = {
    pendente: ["ad-badge--orange", <Clock size={12} />, "Pendente"],
    aprovada: ["ad-badge--blue", <ShieldCheck size={12} />, "Aguarda sinal"],
    confirmada: ["ad-badge--green", <CheckCircle2 size={12} />, "Confirmada"],
    cancelada: ["ad-badge--red", <XCircle size={12} />, "Cancelada"],
    concluida: ["ad-badge--green", <CheckCircle2 size={12} />, "Concluída"],
  };
...
function GalleryModal({ listing, onClose }) {
  const { listListingImages, addListingImage, deleteListingImage } = useAdmin();
  ...
  async function load() {
    setLoading(true);
    try {
      setImages(await listListingImages(listing.id));
    } catch (err) {
      setError(err.message || "Não foi possível carregar as fotos.");
    } finally {
      setLoading(false);
    }
  }
  ...
  function handleFile(e) {
    ...
    reader.onload = async () => {
      setUploading(true);
      try {
        await addListingImage(listing.id, { url: reader.result, tipo: images.length === 0 ? "principal" : "galeria" });
        await load();
      } catch (err) {
        setError(err.message || "Não foi possível adicionar a foto.");
      } finally {
        setUploading(false);
      }
    };
    ...
    <>
      ...
      <div className="ad-modal ad-modal--wide">
        <div className="ad-modal-header">
          <h3>Fotos — {listing.nome}</h3>
          <button className="ad-modal-close" onClick={onClose}><X size={18} /></button>
        </div>
        <div className="ad-modal-body">
          <p style={{ fontSize: "0.85rem", color: "var(--ad-muted)", margin: "0 0 4px" }}>
            Adiciona várias fotos para a galeria de "mais detalhes" deste item no site. A primeira foto é usada como principal.
          </p>
          {error && (
            <p style={{ background: "rgba(220,53,69,.1)", color: "#c0392b", padding: "10px 14px", borderRadius: 10, fontSize: "0.85rem", fontWeight: 700, margin: 0 }}>
              {error}
            </p>
          )}

          <label className="ad-image-upload-btn" style={{ alignSelf: "flex-start", cursor: uploading ? "wait" : "pointer" }}>
            <Upload size={14} /> {uploading ? "A carregar..." : "Adicionar foto"}
            <input type="file" accept="image/*" onChange={handleFile} disabled={uploading} style={{ display: "none" }} />
          </label>

          {loading ? (
            <p className="ad-empty">A carregar fotos...</p>
          ) : images.length === 0 ? (
            <p className="ad-empty">Ainda não há fotos. Adiciona a primeira!</p>
          ) : (
            <div className="ad-gallery-grid">
              {images.map((img) => (
                <div key={img.id} className="ad-gallery-thumb">
                  <img src={img.url} alt={img.titulo || ""} />
                  {img.tipo === "principal" && <span className="ad-gallery-badge">Principal</span>}
                  <button className="ad-gallery-remove" onClick={() => handleDelete(img.id)} title="Remover"><X size={13} /></button>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="ad-modal-footer">
          <button className="ad-btn-primary" onClick={onClose}>Fechar</button>
        </div>
      </div>
    </>
  );
...
function FormModal({ title, onClose, onSave, children, wide, saving, error }) {
  React.useEffect(() => {
  ...
  return (
    <>
      ...
      <div className={`ad-modal ${wide ? "ad-modal--wide" : ""}`}>
        ...
        <div className="ad-modal-body">
          {error && (
            <p style={{ background: "rgba(220,53,69,.1)", color: "#c0392b", padding: "10px 14px", borderRadius: 10, fontSize: "0.85rem", fontWeight: 700, margin: "0 0 4px" }}>
              {error}
            </p>
          )}
          {children}
        ...
        <div className="ad-modal-footer">
          <button className="ad-btn-ghost" onClick={onClose} disabled={saving}>Cancelar</button>
          <button className="ad-btn-primary" onClick={onSave} disabled={saving}>
            <CheckCircle2 size={16} /> {saving ? "A guardar..." : "Guardar"}
          </button>
        </div>
      </div>
    </>
