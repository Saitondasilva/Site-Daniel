...
export default function ServicePage() {
  ...
  return (
    <main className="sp-root">
          ...
          {sorted.map((listing) => (
            ...
            >
              ...
              <div className="sp-card-avatar">
                {listingFace(listing) ? (
                  <img src={listingFace(listing)} alt={listing.nome} />
                ) : (
      ...
      {/* Modal de mais detalhes (galeria de fotos) */}
      {detailListing && (
        <ListingDetailModal listing={detailListing} onClose={() => setDetailListing(null)} renderStars={renderStars} />
      )}
    </main>
  );
}

function ListingDetailModal({ listing, onClose, renderStars }) {
  const { listListingImages } = useAdmin();
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const data = await listListingImages(listing.id);
        setImages(data);
      } catch (_) {
        setImages([]);
      } finally {
  ...
  const mainImage = hasImages ? images[active]?.url : (listing.image || null);

  return (
    <>
      <div className="sp-modal-backdrop" onClick={onClose} />
      <div className="sp-modal" role="dialog" aria-modal="true">
        <button className="sp-modal-close" onClick={onClose} aria-label="Fechar"><X size={20} /></button>

        {!loading && mainImage && (
          <div className="sp-detail-hero">
            <img src={mainImage} alt={listing.nome} />
          </div>
        )}
        {!loading && hasImages && images.length > 1 && (
          <div className="sp-detail-thumbs">
            {images.map((img, i) => (
              <button
                key={img.id}
                className={`sp-detail-thumb ${i === active ? "active" : ""}`}
                onClick={() => setActive(i)}
              >
                <img src={img.url} alt="" />
              </button>
