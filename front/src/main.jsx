...
import {
  ...
  Compass, X, LogOut, User, Package, CalendarCheck,
} from "lucide-react";
...
// Estas categorias existem na base de dados para poderem ser editadas no
// admin como Categorias/Serviços normais, mas têm apresentação própria na
// home (slideshows) em vez de aparecerem na grelha "O que fazemos".
    ...
    <>
      ...
      <nav aria-label="Menu principal" style={{ position:"fixed", top:0, left:0, bottom:0, zIndex:99, width:"min(300px,85vw)", background:"var(--cream)", padding:"28px 24px", display:"flex", flexDirection:"column", gap:"8px", transform:open?"translateX(0)":"translateX(-100%)", transition:"transform 0.28s cubic-bezier(.4,0,.2,1)", overflowY:"auto" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"16px" }}>
          <Link to="/sobre" onClick={onClose} style={{ fontFamily:"'Playfair Display',serif", fontSize:"1.1rem", fontWeight:600, color:"var(--forest-deep)", textDecoration:"none" }}>STP Tours</Link>
        ...
        {user && (
          <div style={{ padding:"12px 14px", borderRadius:"14px", background:"rgba(88,185,87,.1)", border:"1px solid rgba(88,185,87,.2)", marginBottom:"8px" }}>
            ...
            <Link to="/minhas-reservas" onClick={onClose} style={{ display:"flex", alignItems:"center", gap:"7px", color:"var(--forest-deep)", fontSize:"0.82rem", fontWeight:800, textDecoration:"none", marginBottom:"10px" }}>
              <CalendarCheck size={14} /> As minhas reservas
            </Link>
...
function HomePage() {
  ...
  const { user, logout } = useAuth();
  const { publicCategories: categories, loading: categoriesLoading, error: categoriesError } = useAdmin();

  const visibleCategories = useMemo(() => {
    const q = query.trim().toLowerCase();
    return categories
      .filter((c) => !SPECIAL_CATEGORY_IDS.includes(c.id))
  ...
  const destinosItems = useMemo(() => {
    const cat = categories.find((c) => c.id === "destinos-turisticos");
    return (cat?.services || []).map((s) => ({ id: s.id, nome: s.name, local: s.label, descricao: s.description, image: s.image }));
  }, [categories]);

  const pacotesItems = useMemo(() => {
    const cat = categories.find((c) => c.id === "pacotes-oferecidos");
    return (cat?.services || []).map((s) => ({ id: s.id, nome: s.name, duracao: s.label, descricao: s.description, preco: s.preco, image: s.image }));
  }, [categories]);
    ...
    <main>
      ...
      <section className="hero" id="inicio">
        <nav className="nav" aria-label="Navegação principal">
          <Link to="/sobre" className="brand" aria-label="Sobre o STP Tours" title="Sobre nós">
            <img src="public/images/logotipo.jpeg" alt="STP Tours" />
          </Link>
          ...
          {user && (
            <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
              <Link to="/minhas-reservas" aria-label="As minhas reservas"
                style={{ display:"flex", alignItems:"center", gap:"6px", fontSize:"0.82rem", fontWeight:700, color:"rgba(255,255,255,.85)", textDecoration:"none" }}>
        ...
        <div className="heroGrid">
          ...
          <div className="heroVisual" aria-label="Paisagem tropical estilizada">
            ...
            <div className="itinerary">
              ...
              <button onClick={() => navigate("/categorias/pacotes-oferecidos")}>Pacotes Desponiveis <ChevronRight size={16} /></button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="content" id="servicos">
        ...
        {categoriesError && (
          <div style={{ margin: "0 0 24px", padding: "16px 20px", borderRadius: 14, background: "rgba(220,53,69,.08)", border: "1px solid rgba(220,53,69,.25)", color: "#c0392b", fontWeight: 600, fontSize: "0.92rem" }}>
            Não foi possível ligar ao servidor ({categoriesError}). Verifica se o backend está a correr e se a base de dados está configurada.
          </div>
        )}
        {!categoriesError && categoriesLoading && (
          <p style={{ margin: "0 0 24px", color: "var(--muted)", fontWeight: 600 }}>A carregar categorias...</p>
        )}
        {!categoriesError && !categoriesLoading && visibleCategories.length === 0 && (
          <p style={{ margin: "0 0 24px", color: "var(--muted)", fontWeight: 600 }}>
            Ainda não há categorias com serviços para mostrar. Adiciona-as no painel de administração.
          </p>
        )}
      ...
      <div className="pacoteSlide">
        ...
        <div className="pacoteSlideBody">
          ...
          <div className="pacoteSlideFooter">
            ...
            <Link to="/categorias/pacotes-oferecidos" className="pacoteCardBtn">
  ...
  />
      ...
      <footer className="footer" id="contacto">
        ...
        <a href="mailto:info@stpTours.st">info@stpTours.st</a>
