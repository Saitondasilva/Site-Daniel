...
const reservaController = {
    ...
    async criar(req, res) {
        try {
            ...
            const dadosReserva = {
                ...
                parceiro_nome: parceiro_nome || null,
                usuario_nome,
                usuario_email,
    ...
    async atualizarStatus(req, res) {
        try {
            ...
            // Validar status
            const statusValidos = ['pendente', 'aprovada', 'confirmada', 'cancelada', 'concluida'];
            ...
            const transicoesValidas = {
                pendente: ['aprovada', 'confirmada', 'cancelada'],
                aprovada: ['confirmada', 'cancelada', 'pendente'],
    ...
    /**
     * Passo 1 — Admin verifica que há condições e aprova a reserva,
     * enviando uma mensagem ao cliente com o valor do sinal a pagar.
     * PATCH /api/reservas/:id/aprovar
     */
    async aprovar(req, res) {
        try {
            const { id } = req.params;
            const { mensagem_admin, sinal_valor, sinal_percentual } = req.body;
            ...
            if (!['pendente', 'aprovada'].includes(reserva.status)) {
                return res.status(400).json({ success: false, message: 'Só é possível aprovar reservas pendentes' });
            }
            if (!mensagem_admin || !mensagem_admin.trim()) {
                return res.status(400).json({ success: false, message: 'Escreve uma mensagem para o cliente' });
            }

            const atualizada = await Reserva.aprovar(id, { mensagem_admin, sinal_valor, sinal_percentual });
            res.json({ success: true, message: 'Reserva aprovada. O cliente foi notificado para pagar o sinal.', data: atualizada });
        } catch (error) {
            console.error('❌ Erro ao aprovar reserva:', error);
            res.status(500).json({ success: false, message: 'Erro ao aprovar reserva' });
        }
    },

    /**
     * Cliente assinala que já pagou o sinal (fica a aguardar confirmação)
     * PATCH /api/reservas/:id/reportar-pagamento
     */
    async clienteReportarPagamento(req, res) {
        try {
            const { id } = req.params;
            const reserva = await Reserva.findById(id);
            if (!reserva) {
            ...
            if (reserva.status !== 'aprovada') {
                return res.status(400).json({ success: false, message: 'Esta reserva ainda não está aprovada para pagamento' });
            }
            const atualizada = await Reserva.clienteReportarPagamento(id);
            res.json({ success: true, message: 'Obrigado! Vamos confirmar o pagamento em breve.', data: atualizada });
        } catch (error) {
            console.error('❌ Erro ao reportar pagamento:', error);
            res.status(500).json({ success: false, message: 'Erro ao reportar pagamento' });
        }
    },

    /**
     * Passo 2 — Admin confirma que o sinal foi recebido. Só aqui a
     * reserva fica 100% confirmada e marcada.
     * PATCH /api/reservas/:id/confirmar-pagamento
     */
    async confirmarPagamento(req, res) {
        try {
            ...
            if (reserva.status !== 'aprovada') {
                return res.status(400).json({ success: false, message: 'A reserva tem de estar aprovada antes de confirmar o pagamento' });
            }
            const atualizada = await Reserva.confirmarPagamento(id);
            res.json({ success: true, message: 'Pagamento confirmado! Reserva 100% confirmada.', data: atualizada });
        } catch (error) {
    ...
    /**
     * Reservas do próprio cliente (autenticado)
     * GET /api/reservas/minhas?usuario_id=...
     */
    async minhas(req, res) {
        try {
            const { usuario_id, usuario_email } = req.query;
            if (!usuario_id && !usuario_email) {
                return res.status(400).json({ success: false, message: 'usuario_id ou usuario_email é obrigatório' });
            }
            const reservas = await Reserva.findByCliente({ usuario_id, usuario_email });
            res.json({ success: true, data: reservas });
        } catch (error) {
            console.error('❌ Erro ao listar as minhas reservas:', error);
