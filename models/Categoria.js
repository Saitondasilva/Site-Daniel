...
class Categoria {
    static async findAll(ativo = null) {
        ...
        return rows.map(parseCategoria);
    }
    ...
    static async findById(id) {
        const [rows] = await pool.query(`
            ...
            GROUP BY c.id
        `, [id]);
        return parseCategoria(rows[0]);
    }

    static async create(data) {
        const { id, nome, descricao, tagline, icon, cor, ativo, heroImage } = data;
        await pool.query(
            `INSERT INTO categorias (id, nome, descricao, tagline, icon, cor, ativo, hero_image) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [id, nome, descricao || null, tagline || null, icon || null, cor || null, 
    ...
    static async update(id, data) {
        const fields = [];
        const values = [];
        const allowedFields = ['nome', 'descricao', 'tagline', 'icon', 'cor', 'ativo'];
